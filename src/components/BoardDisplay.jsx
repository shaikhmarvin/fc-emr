...
export default function BoardDisplay({
  ...
  const baselineBrowserZoomRef = useRef(window.devicePixelRatio || 1);
  ...
  useEffect(() => {
    const updateBrowserZoomCompensation = () => {
      const currentBrowserZoom = window.devicePixelRatio || 1;
      const baselineBrowserZoom = baselineBrowserZoomRef.current || 1;
      setBrowserZoomCompensation(baselineBrowserZoom / currentBrowserZoom);
    };

    updateBrowserZoomCompensation();

    window.addEventListener("resize", updateBrowserZoomCompensation);

    return () => {
      window.removeEventListener("resize", updateBrowserZoomCompensation);
    };
  }, []);
          ...
          <div className="flex flex-wrap items-start justify-between gap-2 2xl:gap-3">
                ...
                <div className="flex min-w-[360px] flex-1 flex-wrap justify-center gap-1.5 xl:gap-2">
                  {[
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`mb-1 border-b border-slate-600 pb-0.5 text-center text-[0.65rem] font-extrabold uppercase tracking-wide leading-tight xl:text-[0.72rem] ${section.tone}`}
                        >
                          {section.label}
                        </div>
                        ...
                        <div className="grid grid-cols-3 justify-center gap-x-2 gap-y-0.5">
                          {names.slice(0, maxNames).map((name, idx) => (
                            <div
                              ...
                              className={`whitespace-normal break-words text-center font-bold leading-tight text-white ${
                                veryCompactNames
                                  ? "text-[0.62rem] xl:text-[0.7rem]"
                                  : compactNames
                                    ? "text-[0.7rem] xl:text-xs"
                                    : "text-xs xl:text-sm"
                              }`}
                              title={name}
                            >
            ...
            <div className="flex min-w-fit shrink-0 items-start justify-end gap-2 xl:gap-3">
              <div className="min-w-0 rounded-xl bg-slate-800/85 px-3 py-2 text-slate-100 shadow">
                <p className="mb-0.5 text-sm font-bold text-white xl:mb-1 xl:text-base">
                  Connect Here
                </p>
              ...
              return (
                ...
                >
                  <div className="mb-1.5 flex shrink-0 items-center justify-between gap-2">
                    <p className="min-w-0 truncate text-[clamp(1.25rem,1.65vw,1.95rem)] font-black leading-tight">
                      {room.label}
                    </p>
                    <div
                  ...
                  {reservedSpecialty && (
                    <div
                      className="mb-1.5 truncate rounded bg-violet-200 px-2 py-0.5 text-xs font-extrabold text-violet-950"
                      title={`${reservedSpecialty.specialty || reservedSpecialty.label} Reserved`}
                    >
                  ...
                  {occupied ? (
                    <div className="flex min-h-0 flex-1 flex-col gap-1.5">
                      <p
                        className="truncate text-[clamp(1.65rem,2.15vw,2.65rem)] font-black leading-tight"
                        title={getPatientBoardName(primaryPatient)}
                      >
                      ...
                      <div className="space-y-1 rounded-lg px-0 py-0.5 text-slate-950">
                        <p
                          ...
                          title={getStudentBoardName(primaryEncounter.assignedStudent)}
                        >
                          <span className="shrink-0 text-[clamp(0.78rem,0.9vw,1rem)] font-bold tracking-wide">
                            Student:
                          </span>
                          <span className="min-w-0 truncate text-[clamp(0.95rem,1.15vw,1.35rem)] font-semibold">
                        ...
                        {primaryEncounter.assignedUpperLevel && (
                          <p
                            className="flex min-w-0 items-baseline gap-1.5 leading-tight"
                            title={getStudentBoardName(primaryEncounter.assignedUpperLevel)}
                          >
                            <span className="shrink-0 text-[clamp(0.78rem,0.9vw,1rem)] font-bold tracking-wide">
                              Upper:
                            </span>
                          ...
                          <span
                            className={`rounded-full border px-2 py-0.5 text-xs font-extrabold ${
                              VISIT_TYPE_BADGE_STYLES[
                                getEncounterVisitTypeKey(primaryEncounter)
                              ]?.badgeClass
                            }`}
                          >
                            {
                            ...
                            >
                              ...
                              <div className="mt-0.5 truncate text-xs font-bold opacity-90">
                                {encounter.assignedStudent
                                  ? `Student: ${getStudentBoardName(encounter.assignedStudent)}`
                                  : ""}
                                {encounter.assignedUpperLevel
                                  ? `${
                                      encounter.assignedStudent ? " • " : ""
                                    }Upper: ${getStudentBoardName(
                                      encounter.assignedUpperLevel
                                    )}`
                                  : ""}
                              </div>
                    ...
                    <div className="flex flex-1 flex-col items-center justify-center space-y-2 text-center">
                      <p className="text-[clamp(1.35rem,1.85vw,2rem)] font-black opacity-90">
                        Available
                      </p>
                      <p className="text-[clamp(0.9rem,1.1vw,1.15rem)] font-bold opacity-70">
                        No patient assigned
                      </p>
                    </div>
                  )}
