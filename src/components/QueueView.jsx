...
const canMarkSeenBySocialWork =
  ...
  const canUsePharmacyQueue =
    ...
    userRole === "leadership" ||
    (canRefill && userRole !== "attending");
  const isPharmacyQueueMode = queueMode === "pharmacy" && canUsePharmacyQueue;
  const canMarkMedicationsReady = false;
  const canBypassPharmacyReadyForPickup = isPharmacyQueueMode && userRole === "undergraduate";
  const canMarkMedicationsPickedUp = isPharmacyQueueMode && userRole === "undergraduate";
  ...
  function pharmacyStatusBadge(encounter) {
    const status = encounter?.pharmacyStatus || encounter?.pharmacy_status || "waiting";

    if (status === "meds_ready") {
      return (
        <span className="rounded-full border border-emerald-300 bg-emerald-100 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-emerald-900 shadow-sm">
          MEDS READY
        </span>
      );
    }

    if (status === "patient_sent") {
      return (
        <span className="rounded-full border border-cyan-300 bg-cyan-100 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-cyan-900 shadow-sm">
          PATIENT SENT
        </span>
      );
    }
    ...
    if (status === "picked_up") {
      return (
        <span className="rounded-full border border-blue-300 bg-blue-100 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-blue-900 shadow-sm">
          MEDS PICKED UP
        </span>
      );
    }
    ...
    if (status === "meds_not_picked_up") {
      return (
        <span className="rounded-full border border-amber-300 bg-amber-100 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-amber-900 shadow-sm">
          NOT PICKED UP
        </span>
      );
    }
    ...
    return (
      <span className="rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-slate-700 shadow-sm">
        WAITING / NOT READY
      </span>
    );
  }
  ...
  function getRefillNumber(encounter) {
    const intakeData = encounter?.intakeData || encounter?.intake_data || {};

    return (
      encounter?.refillNumber ||
      encounter?.refill_number ||
      intakeData?.refillNumber ||
  ...
  function rowMatchesQueueSearch(patient, encounter) {
    ...
    const refillNumber = normalizeSearchText(getRefillNumber(encounter));
          ...
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-800">
                Read-only for pharmacy, leadership, and refill access. Undergrad can mark when medications are picked up.
              </p>
            </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex flex-wrap items-center gap-2">
                        {getRefillNumber(encounter) && (
                          <span className="rounded-lg bg-purple-700 px-3 py-1 text-sm font-extrabold text-white shadow-sm">
                            R#{getRefillNumber(encounter)}
                          </span>
                        )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["refill_only", "specialty_only"].includes(getEncounterVisitTypeKey(encounter)) && (
                        <span
                          className={`rounded-full border px-2 py-1 text-xs font-semibold ${VISIT_TYPE_BADGE_STYLES[getEncounterVisitTypeKey(encounter)]?.badgeClass
                            }`}
                        >
                    ...
                    {canMarkMedicationsPickedUp && encounter?.pharmacyStatus !== "picked_up" && (
                      <button
                        type="button"
                        onClick={() => onMarkMedicationsPickedUp?.(encounter.id)}
                        className="mt-2 min-h-[44px] w-full rounded-lg bg-blue-600 px-3 py-2 text-sm font-bold text-white hover:bg-blue-700"
                      >
  ...
  return (
    <div className="space-y-4 p-3 sm:p-4 lg:space-y-6 lg:p-6">
      {isPharmacyQueueMode && pharmacyReadyRows.length > 0 && (
