...
export default function WomensHealthSchedule({ entries, settings, canEditSettings, onSaveSettings, onUpdate }) {
  ...
  async function saveClinicianPreference(entry, value) {
    setBusy(true);
    setMessage("");
    try {
      await onUpdate(entry.id, { clinicianGenderPreference: value });
    } catch (error) {
      setMessage(error.message || "Could not save clinician preference.");
    } finally { setBusy(false); }
  }
